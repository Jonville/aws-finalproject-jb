...
const BookclassView = () => {
  ...
  useEffect(() => {
    if(userInfo) {
      setLoad(true);
      const axiosURL = `/api/bookclass/classNo=${no}&userNo=${userInfo.no}`;
      GET(axiosURL)
        .then(res => {
          ...
          <dl className="classBox" id="classBox" ref={classBox}>
            ...
            <dd>
              <ul>
                ...
                <li><RiMapPinFill/><span>{dataList.location + " "}{dataList.locationDetail}{/*<small>({dataList.map})</small>*/}</span></li>
              </ul>
            </dd>
          </dl>
        </section>
