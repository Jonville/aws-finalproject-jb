...
function SubscribersPage() {
  ...
  const [booklogData, setBooklogData] = useState([]);

  const fetchBooklogData = () => {
    axios.get(`/api/booklogs/${no}`)
      .then(response => {
        setBooklogData(response.data);
      })
      .catch(err => {
        console.log(err + ' 서버요청 에러');
      });
    ...
    axios
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          // 변경된 배열 다시 localStorage에 저장
          localStorage.removeItem("subscribedBlogs");
            ...
            <ul className="subscribers-list">
              {data.map((item) => (
                <li key={item.no} className="subscriber-item">
                  <Link to={`/booklog/${item.subed_no.no}`}>
