...
public class OrdersService {
    ...
    public List<OrderResponseDto> getOrderListforAdm() {
        List<OrderResponseDto> resList = ordersRepository.findAll(Sort.by(Sort.Direction.DESC, "createdAt")).stream().map(ord -> {
            if(ord.getGoods() != null) return OrderResponseDto.ofWithGoods(ord);
...
//                Images img = goodsRepository.findById(ord.getGoods().getNo()).get().getImagesList().stream().filter(item -> item.getThumbnailYn().equals("Y")).toList().get(0);
//                return OrderResponseDto.ofWithGoods(ord);
//            }
//            else return OrderResponseDto.ofWithRecycle(ord);
//        }).toList();
//        return resList;
